...
// tmp/09-tooling.md / tmp/12-workers-adapter.md packages/cloudflare の vite.config.ts。
...
//
// `unknown` 経由のキャストの理由は packages/node/vite.config.ts のコメント参照
...
export default defineConfig({
  ...
  // (packages/node/vite.config.ts と同じ理由: vite-plus は rolldown-vite ベースで
