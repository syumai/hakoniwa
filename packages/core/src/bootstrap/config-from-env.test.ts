...
describe("loadConfigFromEnv", () => {
  ...
  it("非推奨のサイト設定の環境変数は siteDefaults (settings 表に値が無いときの既定値) に入る", () => {
    const config = loadConfigFromEnv({
      HAKONIWA_AUTH_SECRET: AUTH_SECRET,
      ...
      HAKONIWA_TIMEZONE: "UTC",
    });
    expect(config.siteDefaults).toEqual({
      title: "テストサイト",
      adminName: "管理者太郎",
      email: "a@example.com",
      bbsUrl: "http://example.com/bbs",
      topPageUrl: "http://example.com/",
      ngWords: ["だめなことば", "もうひとつ"],
      useLbbs: true,
      timezone: "UTC",
    });
  });

  it("サイト設定の環境変数が空文字列なら既定値にフォールバックする (Cloudflare vars 対策)", () => {
    const config = loadConfigFromEnv({
      HAKONIWA_AUTH_SECRET: AUTH_SECRET,
      HAKONIWA_SITE_TITLE: "",
      HAKONIWA_ADMIN_NAME: "",
  ...
  it("廃止した HAKONIWA_UNIT_TIME_SEC / HAKONIWA_START_AT / HAKONIWA_FINAL_TURN は読まない", () => {
    const config = loadConfigFromEnv({
      ...
      HAKONIWA_START_AT: "not a date",
      HAKONIWA_FINAL_TURN: "-1",
    });
    expect(config.game.unitTimeSec).toBe(defaultConfig.unitTimeSec);
    expect(config).not.toHaveProperty("startAt");
    expect(config).not.toHaveProperty("finalTurn");
  });
  ...
  it("不正な数値は Error を投げる", () => {
    expect(() =>
      loadConfigFromEnv({ HAKONIWA_AUTH_SECRET: AUTH_SECRET, HAKONIWA_MAX_CATCH_UP_TURNS: "abc" }),
    ).toThrow(/HAKONIWA_MAX_CATCH_UP_TURNS/);
    expect(() =>
      loadConfigFromEnv({ HAKONIWA_AUTH_SECRET: AUTH_SECRET, HAKONIWA_MAX_CATCH_UP_TURNS: "1.5" }),
    ).toThrow(/HAKONIWA_MAX_CATCH_UP_TURNS/);
  });

  it("0 以下の maxCatchUpTurns は Error を投げる", () => {
    expect(() =>
