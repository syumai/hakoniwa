...
出力される `id` を `wrangler.jsonc` の `kv_namespaces` に追加します:

```jsonc
"kv_namespaces": [{ "binding": "SNAPSHOT", "id": "ここに出力された id を貼る" }],
```

その後 `pnpm deploy` (または `wrangler deploy`) で再デプロイすれば有効になります。

TTL は `wrangler.jsonc` の `vars` に既定値付きで入っているので、必要なら書き換えてください:

- `HAKONIWA_SNAPSHOT_TTL_SEC` (既定 60 秒): 進行中/開始前のゲームのページ、現在のゲームで終了済みの島ページ (掲示板の記帳が入りうるため)。Workers KV の最小 TTL が 60 秒のため、これ未満を指定しても 60 秒に切り上げられます。
...
| `HAKONIWA_SNAPSHOT_TTL_SEC`                                     | `60`                                      | KV スナップショットキャッシュ (任意。上記「KV スナップショットキャッシュ」節) の短期 TTL (秒)。進行中/開始前のゲームのページ、終了済みの島ページに使う。60 未満を指定しても 60 に切り上げられる (Workers KV の最小 TTL)                                               |  -   | vars (任意、SNAPSHOT 使用時のみ意味を持つ) |
| `HAKONIWA_SNAPSHOT_TTL_IMMUTABLE_SEC`                           | `2592000` (30日)                          | KV スナップショットキャッシュの長期 TTL (秒)。過去のゲーム、終了済みのトップページ (不変) に使う                                                                                                                                                                      |  -   | vars (任意、SNAPSHOT 使用時のみ意味を持つ) |
